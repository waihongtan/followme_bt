#ifndef ACTIONDEFLATEOBSTACLE_H
#define ACTIONDEFLATEOBSTACLE_H
...
namespace BT
{
class AsyncActionDeflateObstacle : public AsyncActionNode
{
  public:
    AsyncActionDeflateObstacle(const std::string& name , const NodeConfiguration & config, ros::NodeHandle* nh, BT::Duration deadline_ms = std::chrono::milliseconds(100));

    virtual ~AsyncActionDeflateObstacle()
    {
        halt();
    }
    ...
    {
        return { 
                 BT::OutputPort<int>("global_costmap_gradient"),
                 BT::OutputPort<int>("local_costmap_gradient") };
    }
